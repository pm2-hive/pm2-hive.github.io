...
* `options` - An object with the following options (additional descriptions of these options are [here](http://pm2.keymetrics.io/docs/usage/pm2-doc-single-page/#graceful-reload)):
  ...
  * `error` - (Default: "~/.pm2/logs/app_name.err") The path to a file to append stderr output to. *Note that pm2's internal logs will always be written to ~/.pm2/pm2.log`*
  * `output` - (Default: "~/.pm2/logs/out.log") The path to a file to append stdout output to. *Note that pm2's internal logs will always be written to ~/.pm2/pm2.log`*
